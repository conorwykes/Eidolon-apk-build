...
test("uses exact per-hit combat modifiers and continuous Burst animation", async () => {
  ...
  assert.doesNotMatch(source, /<FiveStarBurstVfx/);
  assert.doesNotMatch(source, /<FiveStarBurstImpactVfx/);
  assert.doesNotMatch(source, /<AttackImpactOverlay/);
  assert.doesNotMatch(source, /<BurstRemotionOverlay/);
  assert.match(source, /lockedTargetIds\.includes\(enemy\.instanceId\)/);
});

test("keeps Zephyra and Solenne's grouped ranged choreography without attack VFX overlays", async () => {
  const source = await readFile(new URL("../app/page.tsx", import.meta.url), "utf8");
  ...
  assert.doesNotMatch(source, /className="zephyra-arrow-flight"/);
  assert.doesNotMatch(source, /className=\{`zephyra-bow-lightning/);
  assert.doesNotMatch(source, /className=\{`solenne-judgement-beam/);
...
test("ships every tiered evolution frame with generous transparent gutters", async () => {
  ...
  const brannockFaceScript = await readFile(new URL("../scripts/prepare-generated-face-sheet.py", import.meta.url), "utf8");
  const brannockFaces = await Promise.all([2, 3, 4, 5].map((stars) =>
    readFile(new URL(`../public/units/battle-faces/brannock-${stars}.webp`, import.meta.url)),
  ));
  assert.match(brannockFaceScript, /remove_magenta_chroma/);
  assert.match(brannockFaceScript, /Image\.new\("RGBA"/);
  assert.equal(new Set(brannockFaces.map((face) => face.toString("base64"))).size, 4, "Brannock needs distinct rarity face art");
  ...
  assert.match(styles, /\.battle-face-portrait\.unit-brannock img\{transform:scale\(1\.32\)!important/);
...
test("cycles only locked 5-star idle sheets with a natural baked VFX loop", async () => {
  ...
  for (const unit of ["kael", "lyra", "brannock", "zephyra", "solenne", "nyx"]) {
    ...
    if (unit === "brannock") {
      assert.equal(record.sourceMode, "authored combined character-and-element frames");
...
test("keeps Remotion cut-ins and moving stages while removing attack and Burst VFX players", async () => {
  const source = await readFile(new URL("../app/page.tsx", import.meta.url), "utf8");
  const player = await readFile(new URL("../app/components/BattleRemotion.tsx", import.meta.url), "utf8");
  ...
  assert.doesNotMatch(compositions, /id="BurstKael"/);
  assert.doesNotMatch(player, /AttackImpactOverlay/);
});

test("keeps grounded enemy stagger while removing elemental impact mounts", async () => {
  const source = await readFile(new URL("../app/page.tsx", import.meta.url), "utf8");
  const styles = await readFile(new URL("../app/globals.css", import.meta.url), "utf8");
  ...
  assert.doesNotMatch(source, /className="enemy-contact-pin"/);
...
test("removes attack and Burst contact overlays while retaining progressive sprite animation", async () => {
  const source = await readFile(new URL("../app/page.tsx", import.meta.url), "utf8");
  const styles = await readFile(new URL("../app/globals.css", import.meta.url), "utf8");
  ...
  assert.doesNotMatch(source, /FiveStarBurstVfx|FiveStarBurstImpactVfx|AttackImpactOverlay/);
  assert.match(source, /PROGRESSIVE_BURST_FRAME_COUNTS/);
  assert.match(source, /spriteSet\.burst\[burstFrame\]/);
});

test("keeps Burst presentation limited to the cinematic cut-in layer", async () => {
  const page = await readFile(new URL("../app/page.tsx", import.meta.url), "utf8");
  const root = await readFile(new URL("../remotion/Root.tsx", import.meta.url), "utf8");
  assert.match(page, /<BurstIntroOverlay/);
  assert.doesNotMatch(page, /ElementalBurstLayer|BurstChargeLayer|BurstFinisherFlourish/);
  assert.doesNotMatch(page, /zephyra-bow-lightning|solenne-judgement-beam/);
  assert.doesNotMatch(root, /BurstVfxComposition/);
});

test("removes every attack and Burst VFX mount while retaining the six cut-ins", async () => {
  const page = await readFile(new URL("../app/page.tsx", import.meta.url), "utf8");
  const player = await readFile(new URL("../app/components/BattleRemotion.tsx", import.meta.url), "utf8");
  const root = await readFile(new URL("../remotion/Root.tsx", import.meta.url), "utf8");
  ...
  assert.doesNotMatch(page, /<FiveStarBurstVfx/);
  assert.doesNotMatch(page, /<FiveStarBurstImpactVfx/);
  assert.doesNotMatch(page, /<AttackImpactOverlay/);
  assert.doesNotMatch(page, /fullscreen-embers\.mp4/);
  assert.match(page, /<BurstIntroOverlay/);
  assert.doesNotMatch(player, /AttackImpactOverlay/);
  assert.doesNotMatch(root, /id="Burst(?:Kael|Lyra|Brannock|Zephyra|Solenne|Nyx)"/);
  for (const unit of ["Kael", "Lyra", "Brannock", "Zephyra", "Solenne", "Nyx"]) {
    assert.match(root, new RegExp(`id: "Intro${unit}"`));
  }
});

test("does not retain the removed five-star VFX component", async () => {
  await assert.rejects(
    readFile(new URL("../app/components/FiveStarBurstVfx.tsx", import.meta.url), "utf8"),
...
test("does not pass the Reduced effects setting into removed combat VFX players", async () => {
  const overlay = await readFile(new URL("../app/components/BattleRemotion.tsx", import.meta.url), "utf8");
  const page = await readFile(new URL("../app/page.tsx", import.meta.url), "utf8");
  assert.doesNotMatch(overlay, /AttackImpactOverlay|BurstRemotionOverlay/);
  assert.doesNotMatch(page, /reducedEffects=\{gameSettings\.reducedEffects\}/);
});

test("does not mount high-frequency compositing effects on normal attacks", async () => {
  const page = await readFile(new URL("../app/page.tsx", import.meta.url), "utf8");
  const overlay = await readFile(new URL("../app/components/BattleRemotion.tsx", import.meta.url), "utf8");
  assert.doesNotMatch(page, /AttackImpactOverlay/);
  assert.doesNotMatch(overlay, /AttackImpactComposition/);
});
